...
const TutorsJob = () => {
    ...
    const currentCards = useMemo(() => getCardsForPage(currentPage), [currentPage]);
    ...
    return (
        <div>
            ...
            <div className='text-white text-center pt-10 md:pb-24 pb-4 bg-[#146DB2]'>
                <h1 className='text-[56px] font-bold'>TUTORS JOBS</h1>
                <p className='text-20px pb-4'>Get the right job you deserve</p>
            </div>

            {/* Cards Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 px-10 py-8">
            ...
            <div className="flex justify-center items-center gap-3 py-6">
                <button onClick={prevPage} disabled={currentPage === 1} className="px-4 py-2 bg-gray-200 rounded disabled:opacity-50">
                    ← Prev
                </button>
                ...
                <button onClick={nextPage} disabled={currentPage === totalPages} className="px-4 py-2 bg-gray-200 rounded disabled:opacity-50">
                    Next →
                </button>
            </div>
        </div>
